...
class _OWPST_jdvu__Front {
	...
	private function __construct( $params ) {
		...
		$PHP_ToLoad = scandir( __OWPST_jdvu__THIS_PLUGIN__FRONT_DIR_ . 'controller'.__OWPST_jdvu__PS_ );
		$PHP_ToLoad = preg_grep( '/-ToLoad\.php$/i' , $PHP_ToLoad );
		sort ( $PHP_ToLoad , SORT_STRING );

		foreach ( $PHP_ToLoad as $key => $fileName ) {
			include( __OWPST_jdvu__THIS_PLUGIN__FRONT_DIR_ . 'controller' . __OWPST_jdvu__PS_ . $fileName );
		}
/*
